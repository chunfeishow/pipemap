...
var DB = function( db_name, size ){
    ...
    return {
        ...
        * 创建表
        */
       CreateTable:function (tableName,param){

          this.query("CREATE TABLE if not exists " + tableName +" ("+param +")",[]);
       },
       /**
        * 插入，回调返回last id
       ...
       showTables:function( table_name, callback){
           this.fetchAll("select * from sqlite_master where type='table' and name like ?", [executeSql], callback);
       },
